...
@section('title')
{{ isset($item) ? 'ویرایش برند' : 'افزودن برند' }}
@stop
...
@section('form')
    ...
    <form id="myForm" action="{{ isset($item) ? route('brand.update', ['brand' => $item['id']]) : route('brand.store')}}" method="post" enctype="multipart/form-data">
        {{ csrf_field() }}
        ...
        <div class="flex center gap10">
            <span onclick="document.location.href = '{{ route('brand.index') }}'" class="btn btn-danger">بازگشت</span>
